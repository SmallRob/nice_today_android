...
// 优化后的日期选择器组件
const OptimizedDatePicker = ({ selectedDate, onDateChange, minDate, maxDate }) => {
  ...
  return (
    <input
      ...
      className="border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm dark:bg-gray-700 dark:text-white w-full"
    />
  );
};
    ...
    <Card title="五行基础知识" className="mb-4">
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
        {Object.entries(fiveElementsInfo).map(([element, info]) => {
          ...
          return (
            <div key={element} className={`text-center p-2 ${bgColorMap[element]} rounded-lg`}>
              <div className={`w-8 h-8 ${circleColorMap[element]} rounded-full mx-auto mb-1 flex items-center justify-center text-white font-bold text-sm`}>
                {element}
              </div>
              <p className="text-xs font-medium whitespace-nowrap text-gray-900 dark:text-white">{info.name}</p>
              <p className="text-xs text-gray-600 dark:text-gray-400 truncate">{info.description}</p>
            </div>
          );
        })}
      </div>
...
// 吉祥颜色卡片组件
const LuckyColorsCard = React.memo(({ luckyColors }) => {
...
// 不宜颜色卡片组件
const UnluckyColorsCard = React.memo(({ unluckyColors }) => {
...
// 饮食养生指南组件
const DietGuideCard = React.memo(({ foodSuggestions }) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div className="bg-green-50 dark:bg-green-900 dark:bg-opacity-20 border border-green-200 dark:border-green-700 rounded-lg p-3">
          ...
          <div className="space-y-1">
            {foodSuggestions.宜.map((food, index) => (
        ...
        <div className="bg-red-50 dark:bg-red-900 dark:bg-opacity-20 border border-red-200 dark:border-red-700 rounded-lg p-3">
          ...
          <div className="space-y-1">
            {foodSuggestions.忌.map((food, index) => (
...
function DressGuidePage() {
  ...
  // 加载数据
  const loadDressInfo = useCallback(async () => {
    setLoading(true);
    setError(null);
    ...
    return (
      <div className="flex-1 overflow-auto bg-white dark:bg-gray-900 pb-safe-bottom">
        <div className="flex flex-col items-center justify-center h-64">
          ...
          <p className="text-gray-600 dark:text-gray-400">正在为您分析今日五行能量...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex-1 overflow-auto bg-white dark:bg-gray-900 pb-safe-bottom">
        <div className="flex flex-col items-center justify-center h-64 p-4">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-center">暂时无法获取穿衣建议数据</p>
          <button 
            onClick={loadDressInfo} 
  ...
  return (
    <div className="flex-1 overflow-auto bg-white dark:bg-gray-900 pb-safe-bottom">
      <div className="p-4 space-y-4">
        ...
        {/* 吉祥颜色详细指南 */}
        <LuckyColorsCard luckyColors={luckyColors} />

        {/* 不宜颜色警示 */}
        <UnluckyColorsCard unluckyColors={unluckyColors} />

        {/* 饮食养生指南 */}
        <DietGuideCard foodSuggestions={selectedDressInfo.food_suggestions} />
