const KlineChart = ({ data, hoveredAge, onHoverAge, theme, chartType, timeDimension, selectedYear }) => {
  ...
  const padding = isMobile ? { top: 35, right: 15, bottom: 45, left: 40 } : { top: 40, right: 20, bottom: 50, left: 50 };
  ...
  const getDisplayData = () => {
    if (timeDimension === 'year') {
      ...
      for (let offset = -12; offset <= 12; offset++) {
        const targetAge = Math.max(0, Math.min(100, currentAge + offset));
  ...
  if (timeDimension === 'year') {
    ...
    for (let age = 0; age <= 100; age += 10) {
      const index = Math.floor((age / 100) * maxIndex);
      ...
      <div className="flex justify-between items-center mb-3">
        <div>
          ...
          <p className={`${isMobile ? 'text-[10px]' : 'text-xs'} mt-1 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
            {timeDimension === 'year' ? '0-100岁综合能量指数' :
