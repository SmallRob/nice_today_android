...
const HoroscopeTab = () => {
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-br from-white via-gray-50 to-gray-100 dark:from-black dark:via-gray-900 dark:to-black overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto hide-scrollbar bg-white dark:bg-black -webkit-overflow-scrolling-touch scroll-container-optimized touch-optimized">
        {/* 防止iOS Safari弹性滚动 */}
        <div className="prevent-bounce-scroll" 
          onTouchStart={(e) => {
            const element = e.currentTarget;
            element.setAttribute('data-start-y', e.touches[0].clientY.toString());
          }}
          onTouchMove={(e) => {
            const element = e.currentTarget;
            const startY = parseInt(element.getAttribute('data-start-y') || '0', 10);
