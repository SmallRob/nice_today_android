  ...
  return (
    <div className="space-y-4 performance-optimized pb-4 shadow-inner app-container">
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-2xl p-3 shadow-lg dark:from-indigo-800 dark:to-purple-900">
        <h2 className="text-base md:text-lg font-bold mb-1">铁板神数模拟抽算</h2>
      ...
      {baziData && activeTab === 'calculation' && (
        <div className="space-y-4">
          ...
          {calculationResult && (
            <div id="clause-display-section" className="bg-white dark:bg-gray-800 rounded-2xl p-3 md:p-4 shadow-sm border border-gray-100 dark:border-gray-700">
              <h2 className="text-base md:text-lg font-bold mb-3">神数条文抽取与解读</h2>
              <ClauseDisplay
      ...
      {!baziData && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-3 md:p-4 shadow-sm border border-gray-100 dark:border-gray-700">
          <div className="text-center py-6 md:py-10">
            ...
            <h3 className="text-base md:text-xl font-bold mb-3">铁板神数演示</h3>
            <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400 mb-4 md:mb-6">请输入八字信息开始模拟抽算</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-md mx-auto">
              <div className="flex items-center space-x-2 md:space-x-3 p-2 md:p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl">
                ...
                <span className="text-xs md:text-sm font-medium">输入八字或随机生成</span>
              </div>
              <div className="flex items-center space-x-2 md:space-x-3 p-2 md:p-3 bg-purple-50 dark:bg-purple-900/20 rounded-xl">
                <span className="text-lg md:text-2xl">②</span>
                <span className="text-xs md:text-sm font-medium">皇极起数算法模拟</span>
              </div>
              <div className="flex items-center space-x-2 md:space-x-3 p-2 md:p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl">
                <span className="text-lg md:text-2xl">③</span>
                <span className="text-xs md:text-sm font-medium">万条文库抽取演示</span>
              </div>
              <div className="flex items-center space-x-2 md:space-x-3 p-2 md:p-3 bg-purple-50 dark:bg-purple-900/20 rounded-xl">
                <span className="text-lg md:text-2xl">④</span>
                <span className="text-xs md:text-sm font-medium">条文详细解读说明</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-slate-800 dark:to-slate-900 rounded-2xl p-3 md:p-4 shadow-sm border border-indigo-100 dark:border-slate-700">
        <h3 className="text-base md:text-lg font-bold mb-3">铁板神数简介</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="bg-white/60 dark:bg-gray-800/60 p-3 rounded-xl">
            <h4 className="font-bold text-indigo-700 dark:text-indigo-300 mb-2 text-sm md:text-base">源流</h4>
            <p className="text-xs md:text-sm text-gray-700 dark:text-gray-300">相传为北宋邵雍（邵康节）所创，是古代最高层次的命理推算术之一。</p>
          </div>
          <div className="bg-white/60 dark:bg-gray-800/60 p-3 rounded-xl">
            <h4 className="font-bold text-indigo-700 dark:text-indigo-300 mb-2 text-sm md:text-base">特点</h4>
            <p className="text-xs md:text-sm text-gray-700 dark:text-gray-300">以八字为基础，通过"皇极起数法"将命运化为卦数，在万条文库中定位命运断辞。</p>
          </div>
          <div className="bg-white/60 dark:bg-gray-800/60 p-3 rounded-xl">
            <h4 className="font-bold text-indigo-700 dark:text-indigo-300 mb-2 text-sm md:text-base">皇极起数</h4>
            <p className="text-xs md:text-sm text-gray-700 dark:text-gray-300">核心算法，将八字天干地支转化为先天八卦数，再演算得到条文编号。</p>
          </div>
          <div className="bg-white/60 dark:bg-gray-800/60 p-3 rounded-xl">
            <h4 className="font-bold text-indigo-700 dark:text-indigo-300 mb-2 text-sm md:text-base">万条文库</h4>
            <p className="text-xs md:text-sm text-gray-700 dark:text-gray-300">据说有12000条或更多条文，每条对应特定命运特征，准确度极高。</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-2xl p-3 shadow-md">
        <p className="text-xs md:text-sm font-medium mb-1">铁板神数被称为"数术之王"，是邵雍易学的最高成就之一</p>
