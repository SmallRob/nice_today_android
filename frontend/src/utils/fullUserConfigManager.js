/**
 * 炫彩版用户配置管理器
 * 独立管理炫彩版的用户配置，与轻量版完全隔离
 */

// 炫彩版默认配置模板
const FULL_DEFAULT_CONFIG = Object.freeze({
  nickname: '炫彩版用户',
  realName: '',
...
// 炫彩版专用存储键名
const FULL_STORAGE_KEYS = {
  USER_CONFIGS: 'nice_today_full_user_configs',
  ACTIVE_CONFIG_INDEX: 'nice_today_full_active_config_index'
};
...
function deepCloneConfig(sourceConfig) {
  ...
  const safeConfig = {
    ...
    birthLocation: sourceConfig.birthLocation ? {
      ...
      lng: sourceConfig.birthLocation.lng ?? 121.47,
      lat: sourceConfig.birthLocation.lat ?? 31.23
    } : null,
...
class FullUserConfigManager {
  ...
  async initialize() {
    try {
      // 从本地存储加载炫彩版专用配置
      const storedConfigs = localStorage.getItem(FULL_STORAGE_KEYS.USER_CONFIGS);
      const storedIndex = localStorage.getItem(FULL_STORAGE_KEYS.ACTIVE_CONFIG_INDEX);

      console.log('炫彩版：从本地存储加载配置:', {
      ...
      this.configs = this.configs.map(config => ({
        nickname: config.nickname || FULL_DEFAULT_CONFIG.nickname,
        realName: config.realName || '',
        birthDate: config.birthDate || FULL_DEFAULT_CONFIG.birthDate,
        birthTime: config.birthTime || FULL_DEFAULT_CONFIG.birthTime,
        shichen: config.shichen || FULL_DEFAULT_CONFIG.shichen,
        birthLocation: config.birthLocation || FULL_DEFAULT_CONFIG.birthLocation,
        zodiac: config.zodiac || FULL_DEFAULT_CONFIG.zodiac,
        zodiacAnimal: config.zodiacAnimal || FULL_DEFAULT_CONFIG.zodiacAnimal,
        gender: config.gender || FULL_DEFAULT_CONFIG.gender,
        mbti: config.mbti || FULL_DEFAULT_CONFIG.mbti,
      ...
      console.error('炫彩版用户配置管理器初始化失败:', error);
  ...
  saveToStorage() {
    try {
      localStorage.setItem(FULL_STORAGE_KEYS.USER_CONFIGS, JSON.stringify(this.configs));
      localStorage.setItem(FULL_STORAGE_KEYS.ACTIVE_CONFIG_INDEX, this.activeConfigIndex.toString());
    } catch (error) {
      console.error('保存炫彩版配置到本地存储失败:', error);
    }
  }
  ...
  notifyListeners() {
    ...
    this.listeners.forEach(callback => {
      try {
        ...
        console.error('炫彩版配置监听器执行错误:', error);
      }
    });
  }
  ...
  addNewConfig(configData = {}) {
    try {
      ...
      console.error('添加炫彩版新配置失败:', error);
      return false;
    }
  }
...
// 创建全局炫彩版配置管理器实例
const fullUserConfigManager = new FullUserConfigManager();

export { fullUserConfigManager };