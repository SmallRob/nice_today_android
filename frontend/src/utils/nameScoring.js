...
const COMMON_STROKES = {
    ...
    '琳': 13, '菊': 14, '婷': 12, '云': 4, '奇': 8, '慧': 15, '颖': 16, '瑶': 15,
    '珍': 10, '琪': 13, '蕾': 19, '莉': 13, '娜': 10, '倩': 10, '欣': 8, '怡': 9,
    '佳': 8, '欢': 22, '薇': 19, '诗': 13, '涵': 12, '琴': 13, '蓉': 16, '晨': 11,
    '宇': 6, '轩': 10, '泽': 17, '睿': 14, '昊': 8, '宸': 10, '翔': 12, '航': 10,
    '晟': 11, '瑞': 14, '鑫': 24, '磊': 15, '鹏': 19, '煜': 13, '炜': 13, '铭': 14,
    '彦': 9, '泓': 9, '凯': 12, '俊': 9, '豪': 14, '锋': 15, '建': 9, '志': 7,
    '鹤': 21, '龙': 16, '凤': 14, '虎': 8, '豹': 10, '鹰': 24, '雁': 12, '鸿': 17,
    '天': 4, '地': 6, '人': 2, '和': 8, '美': 9, '善': 12, '德': 15, '仁': 4,
    '义': 13, '礼': 18, '智': 12, '信': 9, '忠': 8, '孝': 7, '廉': 13, '耻': 10,
    '春': 9, '夏': 10, '秋': 9, '冬': 5, '风': 9, '雨': 8, '雷': 13, '电': 13,
    '山': 3, '水': 4, '木': 4, '火': 4, '土': 3, '金': 8, '石': 5, '竹': 6,
    '梅': 11, '兰': 23, '竹': 6, '菊': 14, '松': 8, '柏': 9, '桃': 10, '李': 7,
    '杏': 7, '梨': 11, '枣': 12, '栗': 10, '橘': 16, '柚': 9, '桂': 10, '榕': 14,
    '安': 6, '宁': 14, '康': 11, '健': 11, '福': 14, '禄': 13, '寿': 14, '喜': 12,
    '财': 10, '富': 12, '贵': 12, '荣': 14, '华': 14, '昌': 8, '盛': 12, '达': 16,
    '成': 7, '功': 5, '业': 13, '兴': 16, '旺': 8, '发': 12, '展': 10, '进': 15,
    '升': 4, '腾': 20, '飞': 9, '跃': 21, '越': 12, '超': 12, '卓': 8, '越': 12
};

/**
 * 尝试获取字符笔画，如果不在字典中，返回默认值
 * @param {string} char 
 * @returns {number} 康熙笔画数，默认为8（常见汉字平均笔画数）
 */
export const getCharStrokes = (char) => {
    if (!char) return 8;
    // 如果字符不在字典中，返回8作为默认值（常见汉字平均笔画数）
    // 用户可以在界面上手动修改
    return COMMON_STROKES[char] || 8;
};
