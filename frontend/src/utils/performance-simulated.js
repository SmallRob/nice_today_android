...
// 强制使用模拟API的开关
const FORCE_SIMULATED_PERFORMANCE = true; // 设置为true确保始终使用模拟API
...
// 浏览器性能API
const BrowserPerformanceAPI = {
  startTrace: async (traceName) => {
    // 使用浏览器原生Performance API
    const startTime = performance.now();
    const markName = `trace_${traceName.traceName}_${Date.now()}`;
    ...
    return {
      ...
      async stop() {
        ...
        recordMetric('customTrace', this.name, duration);
      ...
      async putMetric(metricName, value) {
        recordMetric('customMetric', `${this.name}_${metricName}`, value);
      },
      async incrementMetric(metricName, increment = 1) {
        recordMetric('customMetric', `${this.name}_${metricName}`, increment, {
          increment: true
        });
      }
    };
...
export const memoryMonitor = () => {
  ...
  const simulatedMemory = {
    used: 20 + Math.round(Math.random() * 30), // 20-50MB
    total: 100,
    limit: 200
  };
  
  recordMetric('memoryUsage', 'current', simulatedMemory.used, simulatedMemory);
...
export const startTrace = async (traceName) => {
  try {
    const PerformanceAPI = await getPerformanceAPI();
    return await PerformanceAPI.startTrace({ traceName });
  } catch (error) {
...
// 导出是否使用模拟API的标志
export const isUsingSimulatedPerformance = () => FORCE_SIMULATED_PERFORMANCE;